...
void AOBasis::addReorderShell( string& start, string& target,  string& shell_type, vector<int>& neworder ) {
    ...
    if ( shell_type.length() == 1 ){
       ...
       if ( shell_type == "F" ){ 
           //votca has no order so far as for gw only the cartesian forms are used, cartesian order is xxx,yyy,zzz,xxy,xyy,xxz,xzz,yyz,yzz,yzz,xyz source: Rohlfing dissertation appendix A
       }
